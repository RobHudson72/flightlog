...
2. **Realtime watching** — Uses [chokidar](https://github.com/paulmillr/chokidar) to watch `<projects>/<project>/*.jsonl` (depth 1, session subtrees ignored), with per-file debounce (30ms) and a sequential drain queue. The initial scan is allowed 90 s to become ready (a large tree under an antivirus scanner takes ~8 s); only if that fails does it fall back to 5-second polling, which is a full-tree scan and costs ~0.2 core per server — the stderr line `file watcher failed, falling back to 5s polling` is the signal to investigate.
